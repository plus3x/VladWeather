...
struct YandexWeatherService {
    static var token: String? { return Environment.YandexWeather.token }
    private static let apiURL = Environment.YandexWeather.apiURL
    private static var session: URLSession? {
        guard let token = YandexWeatherService.token else { return nil }
        
        let configuration = URLSessionConfiguration.default

        configuration.httpAdditionalHeaders = ["X-Yandex-API-Key": token]
